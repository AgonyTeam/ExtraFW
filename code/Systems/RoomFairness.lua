...
function Content:GetGridCountAroundPos(oPos)
	local room = Game():GetRoom()
	local count = 0
	local pos = oPos
	if not Content:IsPosInRoom(pos) or room:GetGridCollisionAtPos(pos) ~= GridCollisionClass.COLLISION_NONE then count = count + 1 end
...
function Content:GetGridCountAroundPlayer()
	local player = Isaac.GetPlayer(0)
	if player.CanFly then
		return 0
	end
	return Content:GetGridCountAroundPos(player.Position)
end

function RoomFairness:TestGrid(pos,doNode)
	local room = Game():GetRoom()
	local v = nil
	if Content:IsPosInRoom(pos) then
		...
		if v ~= nil and doNode then
			RoomFairness:ProcessFillNode(pos)
		end
	end
	return v or 0.0
end

function RoomFairness:ProcessFillNode(pos)
	local room = Game():GetRoom()
	--Is Node in Room
	if not Content:IsPosInRoom(pos) then return end
	...
	nodeFreedom = nodeFreedom + RoomFairness:TestGrid(tpos, true)
	...
	--MIDDLE
	nodeFreedom = nodeFreedom * RoomFairness:TestGrid(pos, false) * 0.25
	
	table.insert(rfdebug,{x=pos.X,y=pos.Y,v=nodeFreedom})
	freedomSum = freedomSum + nodeFreedom
end

function RoomFairness:CalculateGridFairness()
...
function Content:CalcDangerNonEternal(ent)
	local d = 0
...
function RoomFairness:UpdateFairness()
	if recalculateGridFairness then
		...
		RoomFairness:ProcessFillNode(sPos)
		...
		for _,v in pairs(ents) do
			if Content:HasEternalSubtype(v.Type,v.Variant) then
				table.insert(possibleEternals,v:ToNPC())
			elseif v:ToNPC() ~= nil then
				local d = Content:CalcDangerNonEternal(v:ToNPC())
		...
		if ((math.random(0,100) * 0.01) < eternalChance or roomd == true) and roomd ~= false then
			...
			for _,v in pairs(shuffled) do
				local et = Content:getEternal(v.Type,v.Variant)
				...
				if first or (danger + dangerSum) <= randomDanger then
					...
					local d = Content:CalcDangerNonEternal(v)
					v:GetData().fairDebug = d
					dangerSum = dangerSum + d
				end
			end
...
function Content:IsGridFairnessCalculated()
	return roomCalculated
end

function Content:GetGridFairness()
	return freedomSum
end

function Content:RenderGridFairnessDebug()
	local room = Game():GetRoom()
...
function RoomFairness:ResetFairness()
	eternalRooms = {}
	eternalChance = 0
end

function RoomFairness:D()
	local room = Game():GetRoom()
...
ExtraFWMod:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, RoomFairness.CalculateGridFairness)
ExtraFWMod:AddCallback(ModCallbacks.MC_POST_NEW_LEVEL, RoomFairness.ResetFairness)
ExtraFWMod:AddCallback(ModCallbacks.MC_POST_UPDATE, RoomFairness.UpdateFairness)
--Content:AddCallback(ModCallbacks.MC_POST_UPDATE, Content.D)